...
I ran the ENCODE Long RNA-Seq pipeline using my script run_encode_rnaseq.py. The versions of STAR and RSEM that are used in pipeline are 2.5.1b and 1.2.23, respectfully.
After I ran the ENCODE Long RNA-Seq pipeline, I ran my own scripts for RSEM differential expression analysis. The ENCODE pipeline stops at the RSEM step of quantifying expression. I took it a step further by 
calling differential expression at both the gene level and the isoform level. The first step in calling differential expression in RSEM is to generate a data matrix. This was done with the applet that I wrote named rsem-generate-data-matrix, which 
wraps up the RSEM command by the same name. For each patient, I created a gene matrix and an isoform matrix for each treatment vs control. These matrices contain the gene name in the first column, or isoform name in the case if isoform level quantification, and the expected_count in the following colums for both the treatment and control. See the documentation of rsem-generate-data-matrix for more details.

The next step was to run EBSeq. RSEM comeswith a script to call EBSeq, named rsem-run-ebseq. I wrote an applet for that. One of the inputs is the data matrix file from the previous step. I called this applet one for every gene matrix file, and
again for each isoform matrix file in order to call both gene-level and isoform-level differential expression. I ran EBSeq with my script run_encode_rnaseq.py.

Below I paste in the code form my scripts.

run_encode_rnaseq.py:
...
run_ebseq.py
	...
	#		run_gene_ebseq(treatment_folder=bf,control_folder=control_barcode_folder,patient_id=patient_id)
			run_isoform_ebseq(treatment_folder=bf,control_folder=control_barcode_folder,patient_id=patient_id)
